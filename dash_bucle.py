...
model = load_model('modelo_2d_dash.h5')
...
vector_ticker = df.Símbolo

ticker_limpio = list()
for iterador in np.arange(0,len(vector_ticker)):
    ticker = vector_ticker[iterador]
    if "NYSE" in ticker:
        ticker_meter = ticker[len(ticker)-4:len(ticker)-1]
        ticker_meter = ticker_meter.replace(":","")
        ticker_meter = ticker_meter.strip()
        ticker_limpio.append(ticker_meter)
        
        
    else:
        ticker_meter = ticker[len(ticker)-5:len(ticker)-1]
        ticker_meter = ticker_meter.replace(":","")
        ticker_meter= ticker_meter.strip()
        ticker_limpio.append(ticker_meter)


ticker_limpio = np.array(ticker_limpio)
vector_ticker = pd.Series(ticker_limpio)

lista_activos = list()
for iterador in np.arange(0,len(vector_ticker)):
    time.sleep(1)
    try:
        df = c.chartDF(symbol=vector_ticker[iterador], timeframe='3m')[['open','high','low','close']] #llamada a iex cloud
        df = df.iloc[0:50,:]
        reversed_df = df.iloc[::-1]
        lista_activos.append(reversed_df)
    except:
        pass

datos_activos = lista_activos[0]
datos_activos["ticker"]=vector_ticker[0]

for iterador in np.arange(1,len(lista_activos)):
    dato = lista_activos[iterador]
    dato["ticker"] = vector_ticker[iterador]
    datos_activos = datos_activos.append(dato)
